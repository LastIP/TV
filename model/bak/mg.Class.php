		...
		for ($i=0; $i < count($json->data->playList); $i++) { 
            ...
            if ($videoUrl == "" && $audioUrl != "") {
                $url = $audioUrl;
                    ...
                    if (strstr($url, "/audio/20")) {
                        $url = str_replace("/audio/", "/movie/dest/", $url); 

                    }elseif (strstr($url, "/audio/auto/")) {
                        $url = str_replace("/audio/auto/", "/movie/auto/dest/", $url); 
                    }

                $url = str_replace(".mp3", ".mp4", $url); 

                if (Ng::httpCode($url) == 200) {
                    $json->data->playList[$i]->videoUrl = $url;
                }
            }
...
////// 视频下载
    ...
    public function initFile($ng_videoUrl){
        $DB = SQL::Connect();

        $sql = "UPDATE ng_videos SET ng_r2status = 1 WHERE ng_videoUrl = ?";
        $stmt = $DB->prepare($sql);
        $stmt->bind_param('s', $ng_videoUrl);
    ...
    public function download(){
        ...
        while ($stmt->fetch()) {
            ...
            if ($ng_videoUrl != "" && !file_exists($video)) {
                exec("wget $ng_videoUrl -P " . $this->dirNg);
            }
            ///音频下载
            $audio = $this->dirNg . end(explode("/", $ng_audioUrl));
            if ($ng_audioUrl != "" && !file_exists($audio)) {
                exec("wget $ng_audioUrl -P " . $this->dirNg);
            }
            ///截图
            if ($ng_videoUrl != "") {
